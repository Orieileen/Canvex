...
The panel opens with **one-click presets** (tu-zi for chat, apimart for images, fal.ai for re-angling) — the two combinations this project actually runs on; click one and the only thing left to fill in is a key. For any other provider, paste its example `curl` into **Set up from a curl example** and the wizard walks you through it in three steps — no JSON to write, image or video. It sends one real request to work out where the result sits in the response and whether the provider is async (their docs won't tell you), then shows you the image it just generated. The ⚡ button next to a model sends one real minimal generation. When it fails you get both the provider's raw response **and** a line saying which kind of problem it is and what to change — an expired key, an exhausted balance, a model name the provider doesn't know, and a provider that's simply down all look alike in a raw error, and only one of them is fixed by editing a field.
