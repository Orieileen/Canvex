...
def create_image_edit_job(*, scene, image_file=None, image_files=None, validated):
    ...
    """
    prompt = validated["prompt"].strip() or (
        "Refine the image while preserving content and layout."
    )
...
def create_split_jobs(*, scene, image_file):
    ...
    with transaction.atomic():
        # Background leg (primary): 用 SPLIT_INPAINT_PROMPT 系统级 prompt.
        background = ImageEditJob.objects.create(
            scene=scene,
            prompt=SPLIT_INPAINT_PROMPT,
            size=_AUTO_SIZE,
            num_images=1,
            is_cutout=False,
        ...
        # Cutout leg (secondary): prompt 留空, is_cutout=True. split_partner 指 background.
