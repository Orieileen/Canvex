...
把供应商文档里的示例 `curl` 粘进**「从一段 curl 开始配」**，向导三步带你配完，不用写 JSON。模型行右边的 ⚡ 会真发一次最小生成。失败时除了供应商的原始报文，还会先说一句**这属于哪类问题、该改哪儿** —— key 过期、余额打光、模型名供应商不认、以及人家自己挂了，在原始报错里长得都差不多，而其中只有一种是靠改配置能解决的。
