...
**快捷配置** —— 一键预设，按角色分组。点一下，旁边就地出现一个 key 输入框，填进去就配完了。
...
**聊天用[兔子](https://api.tu-zi.com/)、生图和视频用 [API Mart](https://apimart.ai/)，是这个项目真正在跑的那一对** —— 这里每个功能都是对着它们做出来、验过来的，所以想最快跑通就配这两家。其余几条是为了让你不被绑死在它们身上。换视角只有一个选项，因为那个视角 LoRA 只在 [fal.ai](https://fal.ai/) 上。

API Mart 那两条生图/视频预设建出来自带该家文档里的**全部模型**（各几十个），以及每个模型真实接受的比例、时长和画质档。
