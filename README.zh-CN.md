...
- **聊天即创作** —— 在画布底部聊天框输入提示词，agent 生成一张或多张图片（或一段视频）并落到画布上。对话记录本身就是画布上的一个框，能像别的元素一样拖动、缩放、滚动。（视频落下来的是一张 **「▶ Watch video」卡片**，点开看片 —— 画布本身不是播放器。）
...
- **界面本身是中英双语** —— 侧栏底部有 English / 中文 开关（连 Excalidraw 自己的语言一起切），旁边是一个图文教程。此外还有缩略图小地图、选中元素上方的实时 W×H 尺寸、以及「回到最新元素」。
- **素材库** —— 保存你生成过的所有图片/视频，按画布分组；点缩略图即可重新插回当前画布。
- **每个模型真实收什么，是逐页抄下来的** —— 只出 8 秒的视频模型、最高只到 1K 的生图模型、用「百万像素」而不是「K」计画质的模型：工具栏那几个下拉列的是**你选的这个模型真的接受的值**，因为预设里给它底下每一个模型都带了这张表。没有空转的旋钮，也不会因为超出范围被顶回 400。画布自己那几档（1K / 2K / 4K、5/10/15 秒、16:9 / 9:16 / 1:1）只是「这个模型一条都没报」时的兜底。
- **自带供应商** —— 端点、key、模型名、以及整个请求形状都在界面上填，不在 `.env` 里。想配多少条通道都行，生成时在工具栏挑用哪条：这张图用 Google，下一张用 GPT。（聊天模型**刻意不在**那个选择器里 —— 一个部署只有一个聊天模型，在「通道配置」里设。）
