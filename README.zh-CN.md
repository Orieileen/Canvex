...
面板顶上有**一键预设**(兔子聊天 / apimart 生图)—— 这两条是项目里真跑通过的组合,点一下只剩一把 key 要填。别家供应商:把文档里的示例 `curl` 粘进**「从一段 curl 开始配」**，向导三步带你配完，不用写 JSON —— 图片和视频都走这一条。它会真发一次请求，把结果在回包里的位置、以及**这家是不是异步的**（文档里看不出来）都问出来，最后把刚生成的那张图显示给你看。模型行右边的 ⚡ 会真发一次最小生成。失败时除了供应商的原始报文，还会先说一句**这属于哪类问题、该改哪儿** —— key 过期、余额打光、模型名供应商不认、以及人家自己挂了，在原始报错里长得都差不多，而其中只有一种是靠改配置能解决的。
