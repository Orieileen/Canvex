...
2. **图片生成** —— 「图像」「拆分」两个工具和 agent 生成的图都要它。
3. *（可选）* **视角重渲染** —— 一把 [fal.ai](https://fal.ai) 的 key，给那个 3D 立方体用。
4. *（可选）* **视频生成**。

面板顶上有**一键预设**(tu-zi 聊天 / apimart 生图 / fal.ai 视角重渲染)—— 这两条是项目里真跑通过的组合,点一下只剩一把 key 要填。别家供应商:把文档里的示例 `curl` 粘进**「从一段 curl 开始配」**，向导三步带你配完，不用写 JSON —— 图片和视频都走这一条。它会真发一次请求，把结果在回包里的位置、以及**这家是不是异步的**（文档里看不出来）都问出来，最后把刚生成的那张图显示给你看。模型行右边的 ⚡ 会真发一次最小生成。失败时除了供应商的原始报文，还会先说一句**这属于哪类问题、该改哪儿** —— key 过期、余额打光、模型名供应商不认、以及人家自己挂了，在原始报错里长得都差不多，而其中只有一种是靠改配置能解决的。

每条通道名字左边有一个**状态点**：绿 = 上次调用通了，橙 = 上次失败（展开卡片能看到供应商返回的原文），空心 = 还没调用过。真实生成也会更新它，不只是 ⚡ —— 一条通道哪天悄悄坏了（key 过期、额度打光、供应商换端点），在这里一眼能看见，而不是变成又一次莫名其妙的生成失败。
...
- **通道都不在这里配** —— 聊天、生图、换视角、视频的端点、API key、模型名和每条通道自己的请求参数全部在界面上填：左侧栏「通道配置」。可以配多条通道、多个模型，生成时在工具栏切换。
