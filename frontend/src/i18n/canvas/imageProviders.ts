...
export const imageProviders = {
  en: {
    ...
    add: "New custom provider config",
    addModel: "Add model",
    models: "Models",
    ...
    field: {
      ...
      ratio_scope: "text_only = drop the ratio key when a reference image is sent",
      ratio_param: "body key for the aspect ratio (aspect_ratio / size)",
      upload_path: "upload path — set when the provider won't take base64 (blank = inline)",
      upload_result_path: "field in the upload reply holding the URL",
      flatten_repair: "provider flattens transparency onto black — restore it after the fact",
      protocol: "protocol",
  ...
  zh: {
    ...
    field: {
      ...
      ratio_scope: "text_only = 有参考图时不下发比例键",
      ratio_param: "比例发到哪个键 (aspect_ratio / size)",
      upload_path: "参考图先传到这个路径换 URL;留空 = 内联 base64",
      upload_result_path: "上传回包里哪个字段是那个地址",
      flatten_repair: "这家会把透明背景压平成黑底 —— 收到后还原回透明",
      protocol: "协议",
      timeout: "请求超时(秒)",
